  ...
  return (
    <>
      ...
      <aside
        ...
        bg-white transition-transform duration-300 z-40
        ${isOpen ? "translate-x-0" : "-translate-x-full"} 
        md:translate-x-0 md:static md:block
        md:bg-white/30 md:backdrop-blur-md md:border-r md:border-white/30`}
      >
        <div className="flex flex-col justify-between h-full px-4 py-6 overflow-y-auto">
          {/* Logo */}
          <div>
            <h1 className="text-xl font-bold text-[#7B1E1E] mb-6 px-2 tracking-wide">Admin Panel</h1>

            {/* Menu */}
            {menuGroups.map((group, i) => (
              <div key={i} className="mb-4">
                {group.title && (
                  <p className="text-xs text-gray-500 mb-2 px-2 uppercase tracking-wider font-semibold">
                    {group.title}
                  </p>
                )}
                ...
                {group.items?.map((item) => (
                  <Link
                    ...
                    className={`flex items-center gap-3 px-3 py-2 text-sm rounded-md transition-all duration-200 ease-in-out font-semibold border-l-4
                      ${
                        isActive(item.path)
                          ? "bg-[#7B1E1E]/90 text-white border-[#7B1E1E] shadow"
                          : "hover:bg-[#7B1E1E]/10 hover:text-[#7B1E1E] text-gray-700 border-transparent"
                      }`}
                  >
                ...
                {group.collapsible && (
                  <div className="px-2">
                    <button
                      ...
                      className="flex justify-between items-center w-full text-sm py-2 text-gray-900 hover:text-[#7B1E1E] font-semibold rounded-md transition hover:bg-[#7B1E1E]/10"
                    >
                      ...
                      <span
                        className={`transition-transform duration-300 ${
                          openMenus[group.key] ? "rotate-180" : "rotate-0"
                        }`}
                      >
          ...
          <div className="pt-4 border-t border-gray-200">
            <Link
              to="/logout"
              className="flex items-center gap-3 px-3 py-2 text-sm font-semibold text-red-600 hover:bg-red-100 rounded-md transition"
            >
